...
using AppCommerce.Models.Repositories;
...
namespace AppCommerce.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        {
            List<Product> productList = HttpContext.Session.GetObject<List<Product>>("ProductList") ?? new List<Product>();
            // Store the updated list back in the session
            HttpContext.Session.SetObject("ProductList", productList);

            int itemCount = productList.Count;
            float totalPrix = productList.Sum(product => product.Prix);
            ViewBag.ItemCount = itemCount;
            ViewBag.Products = productList;
            ViewBag.Total = totalPrix;
            return View();
        }

        [AllowAnonymous]
        public IActionResult AddToCart(int id, string designation, float prix, string photo, int quantite )
        {
            List<Product> productList = HttpContext.Session.GetObject<List<Product>>("ProductList") ?? new List<Product>();
            Product newProduct = new Product { Id = id, Désignation = designation, Prix = prix, Image = photo, Quantite = quantite };
